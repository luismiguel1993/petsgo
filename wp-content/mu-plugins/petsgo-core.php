  ...
  <tr>
    ...
    public function print_admin_js() {
        ...
        window.PG = {
            ...
            /* Registro de guías — cada página registra las suyas */
            _guides: {},
            regGuide: function(key, icon, title, body, color){
                PG._guides[key] = {i:icon,t:title,b:body,c:color};
            },
            /**
        ...
        /* ── Multi-select Checklist Widget ── */
            ...
            /* Delegated click handler for guide buttons using data-guide attribute */
            $(document).on('click','.pg-info-guide-btn[data-guide]',function(){
                var g=PG._guides[$(this).data('guide')];
                if(g) PG.showGuide(g.i,g.t,g.b,g.c);
            });
        })(jQuery);
        </script>
        <?php
    ...
    public function page_dashboard() {
        ...
        <?php else: ?>
            <?php if ($is_admin): ?>
                <h1>🐾 PetsGo — Dashboard Analítico <button class="pg-info-guide-btn" data-guide="dashboard" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#00A8E8' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
                <script>PG.regGuide('dashboard','📊','Dashboard Analítico','<h4>📊 Dashboard Analítico</h4><p><strong>¿Qué es?</strong><br>Panel central del administrador. Muestra métricas globales en tiempo real: ventas, pedidos, comisiones, riders y productos.</p><p><strong>Secciones disponibles:</strong></p><ul><li><strong>KPIs principales:</strong> Ventas totales, comisiones, pedidos y tiendas activas.</li><li><strong>Filtros:</strong> Filtra por rango de fechas (hoy, semana, mes, rango personalizado).</li><li><strong>Tablas detalladas:</strong> Riders, tiendas, productos y pedidos recientes.</li><li><strong>Exportar:</strong> Descarga imagen del dashboard o PDF con los datos filtrados.</li></ul><p><strong>Consejo:</strong> Usa los filtros de fecha para comparar períodos y detectar tendencias.</p>','#00A8E8');</script>
                <p class="dash-sub">Métricas globales · <span id="dash-updated"></span></p>
            <?php else: ?>
                <h1>🏪 <?php echo esc_html($store_name); ?> — Dashboard <button class="pg-info-guide-btn" data-guide="vendor-dashboard" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#22C55E' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
                <script>PG.regGuide('vendor-dashboard','🏪','Dashboard de Tienda','<h4>🏪 Dashboard del Vendedor</h4><p><strong>¿Qué es?</strong><br>Panel estadístico individual de la tienda seleccionada. Muestra KPIs y métricas exclusivas de esta tienda.</p><p><strong>Datos disponibles:</strong></p><ul><li><strong>Ventas:</strong> Total vendido, promedio por pedido, productos más vendidos.</li><li><strong>Pedidos:</strong> Cantidad de pedidos y su estado actual.</li><li><strong>Productos:</strong> Stock, productos activos y agotados.</li><li><strong>Comisiones:</strong> Desglose de comisiones generadas.</li></ul><p><strong>Filtros de fecha:</strong> Filtra por hoy, semana, mes o rango personalizado para comparar períodos.</p>','#22C55E');</script>
    ...
    public function page_products() {
        ...
        <div class="wrap petsgo-wrap">
            <h1>🛒 Productos (<span id="pg-total">...</span>) <button class="pg-info-guide-btn" data-guide="products" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#8B5CF6' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('products','🛒','Productos','<h4>🛒 Gestión de Productos</h4><p><strong>¿Qué es?</strong><br>Catálogo completo de productos del marketplace. Permite crear, editar, eliminar y buscar productos.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Agregar producto:</strong> Botón "+ Nuevo" para crear un producto con nombre, descripción, precio, stock, categoría e imágenes.</li><li><strong>Editar:</strong> Clic en el ícono de edición para modificar un producto.</li><li><strong>Eliminar:</strong> Requiere confirmación con contraseña (acción irreversible).</li><li><strong>Activar/Desactivar:</strong> Toggle para controlar visibilidad del producto en el sitio.</li><li><strong>Buscar:</strong> Filtra por nombre, categoría o tienda.</li></ul><p><strong>Imágenes:</strong> Mín 400×400px, máx 2000×2000px, máx 2MB. Formatos: JPG, PNG, WebP.</p>','#8B5CF6');</script>
            <?php if (!$is_admin && $vid): ?>
    ...
    public function page_vendors() {
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🏪 Tiendas (<span id="pv-total">...</span>) <button class="pg-info-guide-btn" data-guide="vendors" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#22C55E' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('vendors','🏪','Tiendas','<h4>🏪 Gestión de Tiendas</h4><p><strong>¿Qué es?</strong><br>Administración de todas las tiendas registradas en el marketplace.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Nueva tienda:</strong> Registra una tienda manualmente con todos sus datos.</li><li><strong>Editar:</strong> Modifica datos, comisiones y estado de una tienda.</li><li><strong>Eliminar:</strong> Elimina la tienda y todos sus productos (requiere contraseña).</li><li><strong>Comisiones:</strong> Ajusta el % de comisión por venta y corte de delivery para cada tienda.</li><li><strong>Estado:</strong> Activa/desactiva tiendas según su suscripción.</li></ul><p><strong>Importante:</strong> Al eliminar una tienda se borran también sus productos, pedidos e imágenes.</p>','#22C55E');</script>
            <div class="petsgo-search-bar">
    ...
    public function page_plans() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📋 Planes (<span id="pp-total">...</span>) <button class="pg-info-guide-btn" data-guide="plans" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#F59E0B' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('plans','📋','Planes','<h4>📋 Planes de Suscripción</h4><p><strong>¿Qué es?</strong><br>Gestión de los planes de suscripción que las tiendas pueden contratar para operar en el marketplace.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Crear plan:</strong> Define nombre, precio mensual, comisión, límite de productos y descripción.</li><li><strong>Editar:</strong> Modifica las condiciones de un plan existente.</li><li><strong>Eliminar:</strong> Elimina un plan (las tiendas suscritas pierden su suscripción).</li></ul><p><strong>Campos del plan:</strong></p><ul><li><strong>Nombre:</strong> Ej: Básico, Profesional, Premium.</li><li><strong>Precio:</strong> Mensualidad en CLP.</li><li><strong>Comisión:</strong> % que PetsGo cobra por cada venta.</li><li><strong>Límite productos:</strong> Máximo de productos que la tienda puede tener.</li><li><strong>Destacado:</strong> Marca visual para resaltar un plan como recomendado.</li></ul>','#F59E0B');</script>
            <div class="petsgo-search-bar">
...
Dashboard con analíticas"></textarea>
    ...
    public function page_invoices() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🧾 Boletas (<span id="bi-total">...</span>) <button class="pg-info-guide-btn" data-guide="invoices" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#06B6D4' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('invoices','🧾','Boletas','<h4>🧾 Gestión de Boletas</h4><p><strong>¿Qué es?</strong><br>Listado de todas las boletas electrónicas generadas en la plataforma.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Ver boleta:</strong> Clic para ver la boleta completa con detalles del pedido.</li><li><strong>Descargar PDF:</strong> Botón de descarga para obtener la boleta en formato PDF.</li><li><strong>Buscar:</strong> Filtra por número de boleta, nombre de cliente o tienda.</li><li><strong>Filtrar por tienda:</strong> Selecciona una tienda específica (solo admin).</li></ul><p><strong>Generación:</strong><br>Las boletas se generan automáticamente desde la sección de Pedidos al hacer clic en 🧾. Cada boleta tiene un número único correlativo por tienda.</p>','#06B6D4');</script>
            <div class="petsgo-search-bar">
    ...
    public function page_invoice_config() {
        ...
        if (!$config_vid && $is_admin) {
            ...
            ?>
            <div class="wrap petsgo-wrap">
                <h1>⚙️ Configuración de Boleta <button class="pg-info-guide-btn" data-guide="invoice-config" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#06B6D4' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
                <script>PG.regGuide('invoice-config','⚙️','Configuración de Boleta','<h4>⚙️ Configuración de Boleta</h4><p><strong>¿Qué es?</strong><br>Ajustes para la generación de boletas electrónicas de cada tienda.</p><p><strong>Campos configurables:</strong></p><ul><li><strong>Razón social:</strong> Nombre legal de la tienda.</li><li><strong>RUT:</strong> Identificación tributaria.</li><li><strong>Giro:</strong> Actividad económica.</li><li><strong>Dirección fiscal:</strong> Domicilio legal.</li><li><strong>Logo boleta:</strong> Imagen que aparece en la boleta PDF.</li><li><strong>Folio inicial:</strong> Número desde donde comienza la numeración.</li></ul><p><strong>Importante:</strong> Todos los campos deben estar completos para poder generar boletas válidas. La configuración es por tienda.</p>','#06B6D4');</script>
    ...
    public function page_audit_log() {
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🔍 Auditoría (<span id="au-total">...</span>) <button class="pg-info-guide-btn" data-guide="audit" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#6366F1' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('audit','🔍','Auditoría','<h4>🔍 Log de Auditoría</h4><p><strong>¿Qué es?</strong><br>Registro histórico de todas las acciones realizadas en la plataforma. Permite rastrear quién hizo qué y cuándo.</p><p><strong>Se registran:</strong></p><ul><li>Creación, edición y eliminación de productos, tiendas y usuarios.</li><li>Cambios de estado en pedidos y entregas.</li><li>Generación de boletas.</li><li>Cambios en configuración y módulos.</li><li>Aprobación/rechazo de riders.</li><li>Intentos de login y cambios de contraseña.</li></ul><p><strong>Filtros:</strong> Busca por nombre de usuario, tipo de acción o descripción. Útil para investigar incidentes o cambios inesperados.</p>','#6366F1');</script>
            <div class="petsgo-search-bar">
...
<p><strong>Afecta a:</strong> Panel de tienda, Checkout, Gestión de cupones admin.</p>'],
    ...
    public function page_legal() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📄 Contenido Legal y Ayuda <button class="pg-info-guide-btn" data-guide="legal" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#6b7280' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <script>PG.regGuide('legal','📄','Contenido Legal','<h4>📄 Contenido Legal y Ayuda</h4><p><strong>¿Qué es?</strong><br>Editor de las páginas legales y de ayuda del sitio público.</p><p><strong>Páginas editables:</strong></p><ul><li><strong>Términos y condiciones:</strong> Reglas de uso de la plataforma.</li><li><strong>Política de privacidad:</strong> Cómo se manejan los datos personales.</li><li><strong>Política de devoluciones:</strong> Procedimiento para cambios y devoluciones.</li><li><strong>Preguntas frecuentes:</strong> FAQ para usuarios.</li><li><strong>Acerca de:</strong> Información sobre PetsGo.</li></ul><p><strong>Editor:</strong> Usa el editor de texto enriquecido para dar formato. Soporta títulos, listas, negrita, links y más.</p><p><strong>Importante:</strong> Los cambios se publican inmediatamente en el sitio al guardar.</p>','#6b7280');</script>
