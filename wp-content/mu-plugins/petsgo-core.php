...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="580" class="email-container" style="max-width:580px;width:100%;">
  ...
  <tr>
    ...
    public function print_admin_css() {
        ...
        /* ── Info Guide Button ── */
        .pg-info-guide-btn{border:none;background:none;cursor:pointer;padding:2px;opacity:0.4;transition:opacity 0.2s;line-height:0;vertical-align:middle;margin-left:6px}
        .pg-info-guide-btn:hover{opacity:1}
        .pg-guide-overlay{position:fixed;inset:0;z-index:170000;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;font-family:Poppins,Segoe UI,sans-serif;animation:pgGuideIn .25s ease}
        .pg-guide-box{background:#fff;border-radius:16px;max-width:540px;width:92%;box-shadow:0 20px 60px rgba(0,0,0,0.3);overflow:hidden;animation:pgGuideScale .25s ease}
        .pg-guide-header{padding:22px 26px 14px;border-bottom:1px solid #f0f0f0;display:flex;align-items:center;justify-content:space-between}
        .pg-guide-header .pg-guide-title{display:flex;align-items:center;gap:10px}
        .pg-guide-header .pg-guide-title span:first-child{font-size:28px}
        .pg-guide-header .pg-guide-title span:last-child{font-size:17px;font-weight:800;color:#2F3A40}
        .pg-guide-close{border:none;background:none;cursor:pointer;padding:4px;color:#9ca3af;font-size:22px;line-height:1}
        .pg-guide-close:hover{color:#2F3A40}
        .pg-guide-body{padding:20px 26px 26px;max-height:62vh;overflow-y:auto;font-size:13px;color:#4b5563;line-height:1.8}
        .pg-guide-body h4{font-size:16px;color:#2F3A40;margin:0 0 12px}
        .pg-guide-body p{margin:0 0 12px}
        .pg-guide-body ul{margin:0 0 12px;padding-left:20px}
        .pg-guide-body li{margin-bottom:4px}
        .pg-guide-body strong{color:#2F3A40}
    ...
    public function print_admin_js() {
        ...
        window.PG = {
            ...
            /**
             * Abre un modal de guía informativa.
             * @param {string} icon - Emoji del encabezado
             * @param {string} title - Título del modal
             * @param {string} body - Contenido HTML de la guía
             * @param {string} color - Color del header (hex)
             */
            showGuide: function(icon, title, body, color){
                $('#pg-guide-modal').remove();
                color = color || '#00A8E8';
                var html = '<div id="pg-guide-modal" class="pg-guide-overlay">'
                    +'<div class="pg-guide-box">'
                    +'<div class="pg-guide-header" style="background:'+color+'10">'
                    +'<div class="pg-guide-title"><span>'+icon+'</span><span>'+PG.esc(title)+'</span></div>'
                    +'<button class="pg-guide-close" title="Cerrar">&times;</button>'
                    +'</div>'
                    +'<div class="pg-guide-body">'+body+'</div>'
                    +'</div></div>';
                $('body').append(html);
                $('#pg-guide-modal').on('click',function(e){if(e.target===this)$(this).remove();});
                $('#pg-guide-modal .pg-guide-close').on('click',function(){$('#pg-guide-modal').remove();});
                $(document).on('keydown.pgGuide',function(e){if(e.key==='Escape'){$('#pg-guide-modal').remove();$(document).off('keydown.pgGuide');}});
            },
            /**
    ...
    public function page_dashboard() {
        ...
        <?php else: ?>
            <?php if ($is_admin): ?>
                <h1>🐾 PetsGo — Dashboard Analítico <button class="pg-info-guide-btn" onclick="PG.showGuide('📊','Dashboard Analítico','<h4>📊 Dashboard Analítico</h4><p><strong>¿Qué es?</strong><br>Panel central del administrador. Muestra métricas globales en tiempo real: ventas, pedidos, comisiones, riders y productos.</p><p><strong>Secciones disponibles:</strong></p><ul><li><strong>KPIs principales:</strong> Ventas totales, comisiones, pedidos y tiendas activas.</li><li><strong>Filtros:</strong> Filtra por rango de fechas (hoy, semana, mes, rango personalizado).</li><li><strong>Tablas detalladas:</strong> Riders, tiendas, productos y pedidos recientes.</li><li><strong>Exportar:</strong> Descarga imagen del dashboard o PDF con los datos filtrados.</li></ul><p><strong>Consejo:</strong> Usa los filtros de fecha para comparar períodos y detectar tendencias.</p>','#00A8E8')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#00A8E8' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
                <p class="dash-sub">Métricas globales · <span id="dash-updated"></span></p>
            <?php else: ?>
                <h1>🏪 <?php echo esc_html($store_name); ?> — Dashboard <button class="pg-info-guide-btn" onclick="PG.showGuide('🏪','Dashboard de Tienda','<h4>🏪 Dashboard del Vendedor</h4><p><strong>¿Qué es?</strong><br>Panel estadístico individual de la tienda seleccionada. Muestra KPIs y métricas exclusivas de esta tienda.</p><p><strong>Datos disponibles:</strong></p><ul><li><strong>Ventas:</strong> Total vendido, promedio por pedido, productos más vendidos.</li><li><strong>Pedidos:</strong> Cantidad de pedidos y su estado actual.</li><li><strong>Productos:</strong> Stock, productos activos y agotados.</li><li><strong>Comisiones:</strong> Desglose de comisiones generadas.</li></ul><p><strong>Filtros de fecha:</strong> Filtra por hoy, semana, mes o rango personalizado para comparar períodos.</p>','#22C55E')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#22C55E' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_products() {
        ...
        <div class="wrap petsgo-wrap">
            <h1>🛒 Productos (<span id="pg-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('🛒','Productos','<h4>🛒 Gestión de Productos</h4><p><strong>¿Qué es?</strong><br>Catálogo completo de productos del marketplace. Permite crear, editar, eliminar y buscar productos.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Agregar producto:</strong> Botón \"+ Nuevo\" para crear un producto con nombre, descripción, precio, stock, categoría e imágenes.</li><li><strong>Editar:</strong> Clic en el ícono de edición para modificar un producto.</li><li><strong>Eliminar:</strong> Requiere confirmación con contraseña (acción irreversible).</li><li><strong>Activar/Desactivar:</strong> Toggle para controlar visibilidad del producto en el sitio.</li><li><strong>Buscar:</strong> Filtra por nombre, categoría o tienda.</li></ul><p><strong>Imágenes:</strong> Mín 400×400px, máx 2000×2000px, máx 2MB. Formatos: JPG, PNG, WebP.</p>','#8B5CF6')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#8B5CF6' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <?php if (!$is_admin && $vid): ?>
    ...
    public function page_vendors() {
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🏪 Tiendas (<span id="pv-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('🏪','Tiendas','<h4>🏪 Gestión de Tiendas</h4><p><strong>¿Qué es?</strong><br>Administración de todas las tiendas registradas en el marketplace.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Nueva tienda:</strong> Registra una tienda manualmente con todos sus datos.</li><li><strong>Editar:</strong> Modifica datos, comisiones y estado de una tienda.</li><li><strong>Eliminar:</strong> Elimina la tienda y todos sus productos (requiere contraseña).</li><li><strong>Comisiones:</strong> Ajusta el % de comisión por venta y corte de delivery para cada tienda.</li><li><strong>Estado:</strong> Activa/desactiva tiendas según su suscripción.</li></ul><p><strong>Importante:</strong> Al eliminar una tienda se borran también sus productos, pedidos e imágenes.</p>','#22C55E')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#22C55E' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <div class="petsgo-search-bar">
    ...
    public function page_orders() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📦 Pedidos (<span id="po-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('📦','Pedidos','<h4>📦 Gestión de Pedidos</h4><p><strong>¿Qué es?</strong><br>Todos los pedidos del marketplace. Permite ver detalles, cambiar estados y generar boletas.</p><p><strong>Estados del pedido:</strong></p><ul><li><strong>🟡 Pago pendiente:</strong> Esperando confirmación de pago.</li><li><strong>🟠 Pendiente:</strong> Pago confirmado, tienda debe preparar.</li><li><strong>🟣 Preparando:</strong> La tienda está preparando el pedido.</li><li><strong>🟢 Listo:</strong> Preparado, esperando rider o retiro.</li><li><strong>🔵 En tránsito:</strong> Rider en ruta hacia el cliente.</li><li><strong>✅ Entregado:</strong> Entregado exitosamente.</li></ul><p><strong>Acciones:</strong></p><ul><li><strong>Cambiar estado:</strong> Avanza el pedido al siguiente estado.</li><li><strong>Generar boleta:</strong> Botón 🧾 para crear la boleta electrónica.</li><li><strong>Filtros:</strong> Por estado, tienda, fecha o búsqueda.</li></ul>','#F97316')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#F97316' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_users() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>👥 Usuarios (<span id="pu-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('👥','Usuarios','<h4>👥 Gestión de Usuarios</h4><p><strong>¿Qué es?</strong><br>Panel de administración de todos los usuarios registrados: clientes, vendedores, riders y administradores.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Nuevo usuario:</strong> Crear manualmente un usuario con rol específico.</li><li><strong>Editar:</strong> Modificar datos personales, rol, dirección y mascotas.</li><li><strong>Eliminar:</strong> Elimina usuario y toda su información (requiere contraseña).</li><li><strong>Cambiar estado:</strong> Activar/suspender cuentas de usuario.</li><li><strong>Filtrar:</strong> Por rol (cliente, vendor, rider, admin) o búsqueda por nombre/email.</li></ul><p><strong>Roles:</strong></p><ul><li><strong>Customer:</strong> Comprador, puede hacer pedidos y dejar reseñas.</li><li><strong>Vendor:</strong> Dueño de tienda, gestiona productos y pedidos.</li><li><strong>Rider:</strong> Repartidor, realiza entregas.</li><li><strong>Admin:</strong> Acceso total al sistema.</li></ul>','#3B82F6')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#3B82F6' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <div class="petsgo-search-bar">
    ...
    public function page_delivery() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🚴 Delivery (<span id="pd-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('🚴','Delivery','<h4>🚴 Gestión de Delivery</h4><p><strong>¿Qué es?</strong><br>Panel completo de gestión de entregas, riders, valoraciones y pagos.</p><p><strong>Secciones (tabs):</strong></p><ul><li><strong>📦 Entregas:</strong> Listado de todas las entregas con estado, rider asignado y detalles.</li><li><strong>🏍️ Riders:</strong> Gestión de riders: aprobar, suspender, ver documentos y estadísticas.</li><li><strong>⭐ Valoraciones:</strong> Reseñas de clientes sobre los riders.</li><li><strong>💰 Pagos:</strong> Historial de pagos a riders y gestión de desembolsos semanales.</li></ul><p><strong>Flujo de entrega:</strong><br>1. Pedido listo → 2. Se asigna rider → 3. Rider recoge → 4. En ruta → 5. Entregado</p><p><strong>Aprobación de riders:</strong><br>Revisa documentos, valida identidad y aprueba/rechaza solicitudes.</p>','#F97316')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#F97316' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <?php if ($is_admin): ?>
    ...
    public function page_plans() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📋 Planes (<span id="pp-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('📋','Planes','<h4>📋 Planes de Suscripción</h4><p><strong>¿Qué es?</strong><br>Gestión de los planes de suscripción que las tiendas pueden contratar para operar en el marketplace.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Crear plan:</strong> Define nombre, precio mensual, comisión, límite de productos y descripción.</li><li><strong>Editar:</strong> Modifica las condiciones de un plan existente.</li><li><strong>Eliminar:</strong> Elimina un plan (las tiendas suscritas pierden su suscripción).</li></ul><p><strong>Campos del plan:</strong></p><ul><li><strong>Nombre:</strong> Ej: Básico, Profesional, Premium.</li><li><strong>Precio:</strong> Mensualidad en CLP.</li><li><strong>Comisión:</strong> % que PetsGo cobra por cada venta.</li><li><strong>Límite productos:</strong> Máximo de productos que la tienda puede tener.</li><li><strong>Destacado:</strong> Marca visual para resaltar un plan como recomendado.</li></ul>','#F59E0B')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#F59E0B' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <div class="petsgo-search-bar">
...
Dashboard con analíticas"></textarea>
    ...
    public function page_invoices() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🧾 Boletas (<span id="bi-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('🧾','Boletas','<h4>🧾 Gestión de Boletas</h4><p><strong>¿Qué es?</strong><br>Listado de todas las boletas electrónicas generadas en la plataforma.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Ver boleta:</strong> Clic para ver la boleta completa con detalles del pedido.</li><li><strong>Descargar PDF:</strong> Botón de descarga para obtener la boleta en formato PDF.</li><li><strong>Buscar:</strong> Filtra por número de boleta, nombre de cliente o tienda.</li><li><strong>Filtrar por tienda:</strong> Selecciona una tienda específica (solo admin).</li></ul><p><strong>Generación:</strong><br>Las boletas se generan automáticamente desde la sección de Pedidos al hacer clic en 🧾. Cada boleta tiene un número único correlativo por tienda.</p>','#06B6D4')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#06B6D4' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <div class="petsgo-search-bar">
            ...
            <div class="petsgo-table-wrap">
            <table class="petsgo-table"><thead id="bi-thead"><tr><th>#</th><th>Nº Boleta</th><th>Pedido</th><th>Tienda</th><th>Cliente</th><th>Total</th><th>Descargas</th><th>Fecha</th><th>Acciones</th></tr></thead>
            <tbody id="bi-body"><tr><td colspan="9" style="text-align:center;padding:30px;color:#999;">Cargando...</td></tr></tbody></table>
            </div>
        </div>
        <script>
        jQuery(function($){
            ...
            var tbl=PG.table({
                ...
                columns:['id','invoice_number','order_id','store_name','customer_name','total_amount','download_count','created_at','_actions'],
                ...
                renderRow:function(b){
                    ...
                    var dc=parseInt(b.download_count)||0;
                    r+='<td style="text-align:center;"><span style="display:inline-block;padding:2px 10px;border-radius:12px;font-size:12px;font-weight:700;'+(dc>0?'background:#e8f5e9;color:#2e7d32':'background:#f3f4f6;color:#9ca3af')+'">'+dc+'</span></td>';
    ...
    public function page_invoice_config() {
        ...
        if ($preview_id) {
            ...
            <div class="wrap petsgo-wrap">
                ...
                <div style="margin-top:16px;display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
                    ...
                    <span style="font-size:13px;color:#6b7280;">📊 Descargada <strong style="color:#374151;"><?php echo intval($inv->download_count ?? 0); ?></strong> <?php echo intval($inv->download_count ?? 0) === 1 ? 'vez' : 'veces'; ?></span>
                </div>
            </div>
            <?php
            return;
        ...
        if (!$config_vid && $is_admin) {
            ...
            ?>
            <div class="wrap petsgo-wrap">
                <h1>⚙️ Configuración de Boleta <button class="pg-info-guide-btn" onclick="PG.showGuide('⚙️','Configuración de Boleta','<h4>⚙️ Configuración de Boleta</h4><p><strong>¿Qué es?</strong><br>Ajustes para la generación de boletas electrónicas de cada tienda.</p><p><strong>Campos configurables:</strong></p><ul><li><strong>Razón social:</strong> Nombre legal de la tienda.</li><li><strong>RUT:</strong> Identificación tributaria.</li><li><strong>Giro:</strong> Actividad económica.</li><li><strong>Dirección fiscal:</strong> Domicilio legal.</li><li><strong>Logo boleta:</strong> Imagen que aparece en la boleta PDF.</li><li><strong>Folio inicial:</strong> Número desde donde comienza la numeración.</li></ul><p><strong>Importante:</strong> Todos los campos deben estar completos para poder generar boletas válidas. La configuración es por tienda.</p>','#06B6D4')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#06B6D4' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_audit_log() {
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🔍 Auditoría (<span id="au-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('🔍','Auditoría','<h4>🔍 Log de Auditoría</h4><p><strong>¿Qué es?</strong><br>Registro histórico de todas las acciones realizadas en la plataforma. Permite rastrear quién hizo qué y cuándo.</p><p><strong>Se registran:</strong></p><ul><li>Creación, edición y eliminación de productos, tiendas y usuarios.</li><li>Cambios de estado en pedidos y entregas.</li><li>Generación de boletas.</li><li>Cambios en configuración y módulos.</li><li>Aprobación/rechazo de riders.</li><li>Intentos de login y cambios de contraseña.</li></ul><p><strong>Filtros:</strong> Busca por nombre de usuario, tipo de acción o descripción. Útil para investigar incidentes o cambios inesperados.</p>','#6366F1')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#6366F1' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <div class="petsgo-search-bar">
    ...
    public function petsgo_download_invoice() {
        ...
        $wpdb->query($wpdb->prepare(
            "UPDATE {$wpdb->prefix}petsgo_invoices SET download_count = COALESCE(download_count,0) + 1 WHERE id = %d", $id
        ));
    ...
    /**
     * Public invoice download by QR token (no login required).
     */
    public function api_public_download_invoice($request) {
        global $wpdb;
        $token = sanitize_text_field($request->get_param('token'));
        $inv = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}petsgo_invoices WHERE qr_token = %s", $token
        ));
        if (!$inv) return new WP_Error('not_found', 'Boleta no encontrada', ['status' => 404]);
        ...
        if (!file_exists($pdf_path)) return new WP_Error('pdf_error', 'No se pudo generar el PDF', ['status' => 500]);

        // Increment download counter
        $wpdb->query($wpdb->prepare(
            "UPDATE {$wpdb->prefix}petsgo_invoices SET download_count = COALESCE(download_count,0) + 1 WHERE id = %d", $inv->id
        ));

        $this->audit('invoice_download_public', 'invoice', $inv->id, $inv->invoice_number . ' (token)');
    ...
    public function page_settings() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>⚙️ Configuración PetsGo <button class="pg-info-guide-btn" onclick="PG.showGuide('⚙️','Configuración','<h4>⚙️ Configuración General</h4><p><strong>¿Qué es?</strong><br>Panel de ajustes globales de la plataforma PetsGo.</p><p><strong>Secciones configurables:</strong></p><ul><li><strong>Datos de la plataforma:</strong> Nombre, logo, colores, contacto.</li><li><strong>Comisiones:</strong> Porcentajes por defecto de comisión por venta y delivery.</li><li><strong>Delivery:</strong> Costos base, distancia máxima, tarifas por km.</li><li><strong>Emails:</strong> Configuración de correos automáticos (bienvenida, stock bajo, boletas).</li><li><strong>Redes sociales:</strong> Links a Instagram, Facebook, etc.</li><li><strong>SEO:</strong> Meta tags y descripciones para el sitio.</li></ul><p><strong>Importante:</strong> Los cambios se guardan inmediatamente al hacer clic en \"Guardar\". Revisa cada campo cuidadosamente antes de guardar.</p>','#2F3A40')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#2F3A40' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
...
<p><strong>Afecta a:</strong> Panel de tienda, Checkout, Gestión de cupones admin.</p>'],
    ...
    public function page_email_preview() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📧 Vista Previa de Correos <button class="pg-info-guide-btn" onclick="PG.showGuide('📧','Vista Previa de Correos','<h4>📧 Vista Previa de Correos</h4><p><strong>¿Qué es?</strong><br>Herramienta para previsualizar cómo se ven los correos automáticos que envía la plataforma.</p><p><strong>Tipos de correo:</strong></p><ul><li><strong>Boleta:</strong> Email con la boleta electrónica adjunta que recibe el cliente.</li><li><strong>Stock bajo:</strong> Alerta al vendedor cuando un producto tiene pocas unidades.</li><li><strong>Stock agotado:</strong> Notificación urgente cuando un producto llega a 0 unidades.</li></ul><p><strong>Uso:</strong> Selecciona el tipo de correo en las pestañas para ver la plantilla renderizada con datos de ejemplo. Útil para verificar que los emails se ven correctamente antes de enviarlos.</p>','#EC4899')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#EC4899' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function ensure_rider_tables() {
        ...
        // ── Invoices table: ensure download_count column exists ──
        if (is_array($inv_cols) && !in_array('download_count', $inv_cols)) {
    ...
    public function register_api_endpoints() {
        ...
        // Invoice public download by token
        register_rest_route('petsgo/v1','/invoice/download/(?P<token>[a-f0-9\-]+)',['methods'=>'GET','callback'=>[$this,'api_public_download_invoice'],'permission_callback'=>'__return_true']);
        // Vendor Lead (public)
    ...
    public function api_validate_invoice($request) {
        ...
        return rest_ensure_response([
            ...
            'download_count' => (int)($inv->download_count ?? 0),
    ...
    public function page_leads() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📩 Leads de Tiendas (<span id="leads-total">...</span>) <button class="pg-info-guide-btn" onclick="PG.showGuide('📩','Leads','<h4>📩 Leads de Tiendas</h4><p><strong>¿Qué es?</strong><br>Solicitudes de registro de nuevas tiendas que quieren vender en PetsGo.</p><p><strong>Flujo del lead:</strong></p><ul><li><strong>Nuevo:</strong> La tienda completó el formulario de registro.</li><li><strong>Contactado:</strong> El equipo se comunicó con el interesado.</li><li><strong>Aprobado:</strong> Se creó la cuenta de tienda en la plataforma.</li><li><strong>Rechazado:</strong> No cumple requisitos o no responde.</li></ul><p><strong>Acciones:</strong></p><ul><li><strong>Cambiar estado:</strong> Actualiza el estado del lead según el seguimiento.</li><li><strong>Ver detalles:</strong> Nombre de tienda, contacto, email, teléfono y plan de interés.</li><li><strong>Buscar:</strong> Filtra por nombre, email o estado.</li></ul><p><strong>Paginación:</strong> Navega entre páginas con los controles inferiores.</p>','#22C55E')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#22C55E' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
            <p class="petsgo-info-bar">Tiendas interesadas que completaron el formulario de contacto en el frontend.</p>

            <div class="petsgo-search-bar">
                <input type="text" id="leads-search" placeholder="🔍 Buscar por tienda, contacto, email..." autocomplete="off">
                ...
                <button type="button" class="petsgo-btn petsgo-btn-primary petsgo-btn-sm" id="leads-btn-search">🔍 Buscar</button>
                <span class="petsgo-loader" id="leads-loader"><span class="spinner is-active" style="float:none;margin:0;"></span></span>
            </div>

            <div class="petsgo-table-wrap">
            <table class="petsgo-table"><thead id="leads-thead"><tr><th>Tienda</th><th>Contacto</th><th>Email</th><th>Teléfono</th><th>Plan</th><th>Estado</th><th>Fecha</th><th>Acciones</th></tr></thead>
            <tbody id="leads-body"><tr><td colspan="8" style="text-align:center;padding:30px;color:#999;">Cargando...</td></tr></tbody></table>
            </div>
        </div>
        ...
        jQuery(function($){
            ...
            var tbl=PG.table({
                thead:'#leads-thead',body:'#leads-body',perPage:25,defaultSort:'id',defaultDir:'desc',
                columns:['store_name','contact_name','email','phone','plan_name','status','created_at','_actions'],
                emptyMsg:'No se encontraron leads.',
                onTotal:function(n){$('#leads-total').text(n);},
                renderRow:function(l){
                    var r='<tr>';
                    r+='<td><strong>'+PG.esc(l.store_name)+'</strong>'+(l.comuna?'<br><small style="color:#888;">'+PG.esc(l.comuna)+'</small>':'')+'</td>';
                    r+='<td>'+PG.esc(l.contact_name)+'</td>';
                    r+='<td><a href="mailto:'+PG.esc(l.email)+'">'+PG.esc(l.email)+'</a></td>';
                    r+='<td>'+PG.esc(l.phone)+'</td>';
                    r+='<td>'+PG.esc(l.plan_name||'—')+'</td>';
                    r+='<td>'+statusBadge(l.status)+'</td>';
                    r+='<td style="font-size:12px;color:#888;white-space:nowrap;">'+(l.created_at||'').substring(0,10)+'</td>';
                    r+='<td><button class="petsgo-btn petsgo-btn-sm" onclick=\'openLead('+JSON.stringify(l).replace(/\'/g,"&#39;")+')\'>✏️ Gestionar</button></td>';
                    r+='</tr>';
                    return r;
                }
            });
            ...
            // Initial load
            searchLeads();
            var st;$('#leads-search,#leads-status-filter').on('input change',function(){clearTimeout(st);st=setTimeout(searchLeads,400);});
            $('#leads-btn-search').on('click',searchLeads);
        });
        </script>
        <?php
    }
    ...
    public function page_categories() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📂 Categorías del Marketplace <button class="pg-info-guide-btn" onclick="PG.showGuide('📂','Categorías','<h4>📂 Categorías del Marketplace</h4><p><strong>¿Qué es?</strong><br>Gestión de las categorías de productos disponibles en la plataforma.</p><p><strong>Acciones disponibles:</strong></p><ul><li><strong>Crear categoría:</strong> Define nombre, ícono/emoji y orden de aparición.</li><li><strong>Editar:</strong> Modifica nombre o ícono de una categoría existente.</li><li><strong>Eliminar:</strong> Elimina la categoría. Los productos asociados quedan sin categoría.</li><li><strong>Reordenar:</strong> Cambia el orden en que aparecen en el menú del sitio.</li></ul><p><strong>Categorías actuales:</strong> Alimento, Juguetes, Salud, Accesorios, Higiene, Ropa, Camas, Transporte, Snacks, Farmacia.</p><p><strong>Importante:</strong> Las categorías se reflejan directamente en el menú de navegación del sitio público.</p>','#8B5CF6')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#8B5CF6' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_coupons() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>🎟️ Cupones de Descuento <button class="pg-info-guide-btn" onclick="PG.showGuide('🎟️','Cupones','<h4>🎟️ Cupones de Descuento</h4><p><strong>¿Qué es?</strong><br>Gestión centralizada de todos los cupones de descuento de la plataforma.</p><p><strong>Tipos de descuento:</strong></p><ul><li><strong>Porcentaje (%):</strong> Descuenta un porcentaje del total.</li><li><strong>Monto fijo ($):</strong> Descuenta una cantidad fija en CLP.</li></ul><p><strong>Configuración:</strong></p><ul><li><strong>Código:</strong> Texto que el cliente ingresa (ej: DESCUENTO20).</li><li><strong>Tienda:</strong> A qué tienda aplica (o todas).</li><li><strong>Mínimo de compra:</strong> Monto mínimo para poder usar el cupón.</li><li><strong>Usos máximos:</strong> Límite total de usos.</li><li><strong>Fecha expiración:</strong> Hasta cuándo es válido.</li></ul><p><strong>Acciones:</strong> Crear, editar, eliminar cupones. Los cupones vencidos se muestran en gris.</p>','#EC4899')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#EC4899' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_legal() {
        ...
        ?>
        <div class="wrap petsgo-wrap">
            <h1>📄 Contenido Legal y Ayuda <button class="pg-info-guide-btn" onclick="PG.showGuide('📄','Contenido Legal','<h4>📄 Contenido Legal y Ayuda</h4><p><strong>¿Qué es?</strong><br>Editor de las páginas legales y de ayuda del sitio público.</p><p><strong>Páginas editables:</strong></p><ul><li><strong>Términos y condiciones:</strong> Reglas de uso de la plataforma.</li><li><strong>Política de privacidad:</strong> Cómo se manejan los datos personales.</li><li><strong>Política de devoluciones:</strong> Procedimiento para cambios y devoluciones.</li><li><strong>Preguntas frecuentes:</strong> FAQ para usuarios.</li><li><strong>Acerca de:</strong> Información sobre PetsGo.</li></ul><p><strong>Editor:</strong> Usa el editor de texto enriquecido para dar formato. Soporta títulos, listas, negrita, links y más.</p><p><strong>Importante:</strong> Los cambios se publican inmediatamente en el sitio al guardar.</p>','#6b7280')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#6b7280' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
    ...
    public function page_chatbot_config() {
        ...
        <div class="wrap petsgo-wrap">
            <h1>🤖 Configuración del Chatbot IA <button class="pg-info-guide-btn" onclick="PG.showGuide('🤖','Chatbot IA','<h4>🤖 Configuración del Chatbot IA</h4><p><strong>¿Qué es?</strong><br>Panel de configuración del asistente inteligente PetBot que ayuda a los usuarios en el sitio.</p><p><strong>Configuraciones:</strong></p><ul><li><strong>Contexto del bot:</strong> Define la personalidad, tono y conocimientos del chatbot.</li><li><strong>Productos destacados:</strong> Productos que el bot puede recomendar.</li><li><strong>Respuestas predefinidas:</strong> Respuestas rápidas para preguntas comunes.</li><li><strong>Modelo IA:</strong> Selección del modelo de inteligencia artificial a usar.</li></ul><p><strong>Importante:</strong> El chatbot consume tokens de API. Monitorea el uso para controlar costos. Desactiva desde Módulos si necesitas pausar el servicio.</p>','#06B6D4')" title="Guía"><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='#06B6D4' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg></button></h1>
